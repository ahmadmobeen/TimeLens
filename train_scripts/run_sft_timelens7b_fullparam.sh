...
#   * FULL-PARAMETER finetune variant (A6): --lora_enable False + --freeze_llm False; vision tower + merger stay frozen.
...
learning_rate=1e-4
master_port=29500
gpu_include=""   # physical GPU id; deepspeed ignores CUDA_VISIBLE_DEVICES, needs --include localhost:N
lora_rank=64
lora_alpha=64
...
if [ -n "${gpu_include}" ]; then
  ds_gpu_args=(--include "localhost:${gpu_include}")
else
  ds_gpu_args=(--num_gpus "${num_devices}")
fi
deepspeed --master_port "${master_port}" "${ds_gpu_args[@]}" training/train/train_sft_timelens.py \
  --bf16 True \
  --fp16 False \
